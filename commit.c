...
int commit_create(const char *message, ObjectID *commit_id_out) {
    ...
    // 2. Read current HEAD as parent (may not exist for the first commit).
    ObjectID parent_id;
    ...
    // 6. Write commit object to the object store.
    ...
    // 7. Update HEAD (branch pointer) to the new commit.
